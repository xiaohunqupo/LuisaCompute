...
#define LUISA_BINDING_GROUP_TEMPLATE_IMPL(TEMPLATE, TEMPLATE2, S, ...)                      \
    ...
    inline void callable_encode_binding_group(CallableInvoke &invoke, Expr<S> s) noexcept { \
        LUISA_MAP(LUISA_BINDING_GROUP_MAKE_INVOKE, __VA_ARGS__)                             \
    }                                                                                       \
    LUISA_MACRO_EVAL(TEMPLATE2())                                                           \
    inline ShaderInvokeBase &operator<<(ShaderInvokeBase &invoke, const S &s) noexcept {    \
        LUISA_MAP(LUISA_BINDING_GROUP_MAKE_INVOKE, __VA_ARGS__)                             \
        return invoke;                                                                      \
    }                                                                                       \
    }                                                                                       \
