...
[[nodiscard]] LUISA_DSL_API Var<float> triangle_interpolate(Expr<float2> bary, Expr<float> a, Expr<float> b, Expr<float> c) noexcept;
[[nodiscard]] LUISA_DSL_API Var<float2> triangle_interpolate(Expr<float2> bary, Expr<float2> a, Expr<float2> b, Expr<float2> c) noexcept;
[[nodiscard]] LUISA_DSL_API Var<float3> triangle_interpolate(Expr<float2> bary, Expr<float3> a, Expr<float3> b, Expr<float3> c) noexcept;
[[nodiscard]] LUISA_DSL_API Var<float4> triangle_interpolate(Expr<float2> bary, Expr<float4> a, Expr<float4> b, Expr<float4> c) noexcept;

// legacy APIs
#define LUISA_INTERPOLATE_DEPRECATED \
    [[deprecated("Use triangle_interpolate instead.")]]

LUISA_INTERPOLATE_DEPRECATED [[nodiscard]] LUISA_DSL_API inline Var<float>
interpolate(Expr<float2> bary, Expr<float> a, Expr<float> b, Expr<float> c) noexcept {
    return triangle_interpolate(bary, a, b, c);
}
LUISA_INTERPOLATE_DEPRECATED [[nodiscard]] LUISA_DSL_API inline Var<float2>
interpolate(Expr<float2> bary, Expr<float2> a, Expr<float2> b, Expr<float2> c) noexcept {
    return triangle_interpolate(bary, a, b, c);
}
LUISA_INTERPOLATE_DEPRECATED [[nodiscard]] LUISA_DSL_API inline Var<float3>
interpolate(Expr<float2> bary, Expr<float3> a, Expr<float3> b, Expr<float3> c) noexcept {
    return triangle_interpolate(bary, a, b, c);
}
LUISA_INTERPOLATE_DEPRECATED [[nodiscard]] LUISA_DSL_API inline Var<float4>
