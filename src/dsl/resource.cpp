...
Var<float4> BindlessTexture2D::sample(Expr<float2> uv) const noexcept {
    ...
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_SAMPLE : CallOp::BINDLESS_TEXTURE2D_SAMPLE,
...
Var<float4> BindlessTexture2D::sample(Expr<float2> uv, Expr<float> mip) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_SAMPLE_LEVEL : CallOp::BINDLESS_TEXTURE2D_SAMPLE_LEVEL,
...
Var<float4> BindlessTexture2D::sample(Expr<float2> uv, Expr<float2> dpdx, Expr<float2> dpdy) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_SAMPLE_GRAD : CallOp::BINDLESS_TEXTURE2D_SAMPLE_GRAD,
...
Var<uint2> BindlessTexture2D::size() const noexcept {
    ...
    return def<uint2>(f->call(
        Type::of<uint2>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_SIZE : CallOp::BINDLESS_TEXTURE2D_SIZE,
        {_array, _index}));
}
...
Var<uint2> BindlessTexture2D::size(Expr<int> level) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<uint2>(f->call(
        Type::of<uint2>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_SIZE_LEVEL : CallOp::BINDLESS_TEXTURE2D_SIZE_LEVEL,
...
Var<float4> BindlessTexture2D::read(Expr<uint2> coord) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE2D_READ : CallOp::BINDLESS_TEXTURE2D_READ,
...
Var<float4> BindlessTexture3D::sample(Expr<float3> uvw) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_SAMPLE : CallOp::BINDLESS_TEXTURE3D_SAMPLE,
...
Var<float4> BindlessTexture3D::sample(Expr<float3> uvw, Expr<float> mip) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_SAMPLE_LEVEL : CallOp::BINDLESS_TEXTURE3D_SAMPLE_LEVEL,
...
Var<float4> BindlessTexture3D::sample(Expr<float3> uvw, Expr<float3> dpdx, Expr<float3> dpdy) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_SAMPLE_GRAD : CallOp::BINDLESS_TEXTURE3D_SAMPLE_GRAD,
...
Var<uint3> BindlessTexture3D::size() const noexcept {
    ...
    return def<uint3>(f->call(
        Type::of<uint3>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_SIZE : CallOp::BINDLESS_TEXTURE3D_SIZE,
        {_array, _index}));
}
...
Var<uint3> BindlessTexture3D::size(Expr<int> level) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<uint3>(f->call(
        Type::of<uint3>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_SIZE_LEVEL : CallOp::BINDLESS_TEXTURE3D_SIZE_LEVEL,
        {_array, _index, level.expression()}));
}
...
Var<float4> BindlessTexture3D::read(Expr<uint3> coord) const noexcept {
    auto f = detail::FunctionBuilder::current();
    return def<float4>(f->call(
        Type::of<float4>(), _is_typed ? CallOp::TYPED_BINDLESS_TEXTURE3D_READ : CallOp::BINDLESS_TEXTURE3D_READ,
        {_array, _index, coord.expression()}));
}
