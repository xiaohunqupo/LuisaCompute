...
#include "common/spectrum_data.h"
...
int main(int argc, char *argv[]) {
    ...
    constexpr auto fourier_cmax = inv_pi;
    {
        ...
        stream << lut_heap.emplace_on_update(SRGB_TO_FOURIER_EVEN, srgb_to_fourier_even, Sampler::linear_linear_mirror()).update() << srgb_to_fourier_even.copy_from(luisa::span{reinterpret_cast<uint*>(lut_data.get()), buffer_size / sizeof(uint)})
               << synchronize();
    }
