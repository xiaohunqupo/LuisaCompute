...
void test_basic_allocate_deallocate() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        LUISA_ASSERT(obj != nullptr, "Allocation failed");
        obj->x = i;
    ...
    LUISA_ASSERT(obj1 != nullptr && obj2 != nullptr, "Re-allocation failed");
...
void test_create_destroy() {
    ...
    
    {
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            LUISA_ASSERT(obj != nullptr, "Creation failed");
            LUISA_ASSERT(obj->value == i, "Constructor not called correctly");
            objects.push_back(obj);
        }
        
        int constructor_count = ComplexStruct::constructor_count.load(std::memory_order_relaxed);
        LUISA_ASSERT(constructor_count == 50, "Constructor count mismatch: expected 50, got {}", constructor_count);
        ...
        LUISA_ASSERT(destructor_count == 50, "Destructor count mismatch: expected 50, got {}", destructor_count);
    }
...
void test_thread_safety() {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            for (int i = 0; i < allocations_per_thread; ++i) {
                auto* obj = pool.allocate();
                LUISA_ASSERT(obj != nullptr, "Thread allocation failed");
                obj->x = t;
    ...
    for (int t = 0; t < num_threads; ++t) {
        for (auto* obj : thread_objects[t]) {
            LUISA_ASSERT(obj->x == t, "Data corruption detected");
        }
    }
...
void test_move_semantics() {
    ...
    LUISA_ASSERT(obj != nullptr, "Allocation from moved pool failed");
...
void test_trivially_destructible() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        LUISA_ASSERT(*p == i, "Value mismatch");
        ints.push_back(p);
    }
