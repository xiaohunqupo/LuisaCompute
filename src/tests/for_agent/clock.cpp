...
int main() {
    ...
    // Test 1: Basic construction and toc() after construction
    {
        ...
        LUISA_ASSERT(elapsed >= 9.0, "Expected elapsed time >= 9ms, got {}ms", elapsed);
        LUISA_ASSERT(elapsed < 100.0, "Expected elapsed time < 100ms, got {}ms (possible system issue)", elapsed);
    ...
    // Test 2: tic() resets the clock
    {
        ...
        LUISA_ASSERT(elapsed >= 4.0 && elapsed < 50.0,
                     "Expected elapsed time ~5ms after tic(), got {}ms", elapsed);
    ...
    // Test 3: Multiple tic/toc cycles
    {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            LUISA_ASSERT(elapsed >= expected - 2.0 && elapsed < expected + 50.0,
                         "Cycle {}: Expected elapsed time ~{}ms, got {}ms", i, expected, elapsed);
    ...
    // Test 4: toc() does not reset the clock (consecutive calls should return increasing values)
    {
        ...
        sleep_ms(5);
        double t2 = clock.toc();
        LUISA_ASSERT(t2 > t1, "Second toc() should be greater than first: t1={}, t2={}", t1, t2);
    ...
    // Test 5: Zero or near-zero elapsed time
    {
        ...
        double elapsed = clock.toc();
        LUISA_ASSERT(elapsed >= 0.0, "Elapsed time should be non-negative, got {}ms", elapsed);
        LUISA_ASSERT(elapsed < 10.0, "Near-zero test elapsed too long: {}ms", elapsed);
    ...
    // Test 6: High precision test (shorter intervals)
    {
        ...
        LUISA_ASSERT(elapsed > 0.0, "1ms sleep should result in positive elapsed time, got {}ms", elapsed);
