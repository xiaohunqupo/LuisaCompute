...
void CUDAExternalExtImpl::cuda_stream_signal(
    ...
    device->with_handle([&] {
        reinterpret_cast<cuda::CUDAEvent *>(cuda_event_handle)
            ->signal(static_cast<CUstream>(cu_stream_ptr), fence_index);
    });
}
...
void CUDAExternalExtImpl::cuda_stream_wait(
    ...
    device->with_handle([&] {
        reinterpret_cast<cuda::CUDAEvent *>(cuda_event_handle)
            ->wait(static_cast<CUstream>(cu_stream_ptr), fence_index);
    });
}
...
void CUDAExternalExtImpl::buffer_copy_async(
    ...
    device->with_handle([&] {
        LUISA_CHECK_CUDA(cuMemcpyDtoDAsync(
            reinterpret_cast<CUdeviceptr>(dst_buffer),
            reinterpret_cast<CUdeviceptr>(src_buffer),
            size, static_cast<CUstream>(stream)));
    });
}

void CUDAExternalExtImpl::sync_stream(void *cu_stream_ptr) {
    device->with_handle([&] {
        LUISA_CHECK_CUDA(cuStreamSynchronize(
